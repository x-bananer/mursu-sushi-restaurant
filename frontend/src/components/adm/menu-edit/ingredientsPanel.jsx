...
export default function IngredientsPanel() {
	...
	const { categories } = useIngredientsCategories();
	...
	const handleSubmit = async (form) => {
		if (mode === "edit" && editing?.id != null) {
			...
			setLocalIngredients((prev) => [tempIngredient, ...prev]);
			...
			try {
				...
				setLocalIngredients((prev) =>
					prev.map((item) =>
						item.id === tempIngredient.id ? created : item,
					),
				);
				...
				setLocalIngredients((prev) =>
					prev.filter((item) => item.id !== tempIngredient.id),
				);
	...
	return (
		<section className="menu-edit__section">
			...
			{/* ───────── STATES ───────── */}
			{loading && <Loader isLight text={t("combo.loading")} />}

			{error && <ErrorState isLight message={error} />}
						...
						<div className="table__row" key={item.id}>
							...
							<span>
								<span className="badge badge--neutral">
									{categoryMap[item.ingredient_type_id] ||
										t("common.unknown")}
								</span>
							</span>
