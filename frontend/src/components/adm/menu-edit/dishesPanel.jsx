...
export default function DishesPanel() {
	...
	const handleSubmit = async (form) => {
		...
		// ───── CREATE ─────
			...
			// optimistic create
			setLocalDishes((prev) => [tempDish, ...prev]);
			...
			try {
				...
				setLocalDishes((prev) =>
					prev.map((item) =>
						item.id === tempDish.id ? created : item,
					),
				);
				...
				setLocalDishes((prev) =>
					prev.filter((item) => item.id !== tempDish.id),
				);
	...
	const handleDelete = async (id) => {
		...
		// optimistic delete
		setLocalDishes((prev) => prev.filter((item) => item.id !== id));
	...
	return (
		<section className="menu-edit__section">
			...
			{/* STATES */}
			{loading && <Loader isLight text={t("admin.loading_dishes")} />}
