...
export default function LiveOrdersColumn({ title, status, orders, onAction }) {
	return (
		<div className="order-col">
			<div className="order-col__header">
				<span
					className={`order-col__dot order-col__dot--${status}`}
				></span>
			...
			<div className="order-col__cards">
				{orders.map((order) => (
					<LiveOrdersCard order={order} onAction={onAction} />
				))}
			</div>
		</div>
	);
