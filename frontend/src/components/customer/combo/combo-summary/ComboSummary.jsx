...
export default function ComboSummary({
	...
	const { addComboToCart, cartActionLoading, cartActionError } =
		useCartContext();
			...
			<div className="combo-summary__container">
				...
				>
					{groupedLayers.topping.map((ingredient, index) => (
						<ComboLayer
							key={`topping-${ingredient.id}-${index}`}
					...
					{groupedLayers.filling.map((ingredient, index) => (
						<ComboLayer
							key={`filling-${ingredient.id}-${index}`}
							ingredient={ingredient}
							index={index}
							moveIngredient={onMoveIngredient}
							removeIngredient={onRemoveIngredient}
				...
				<Button
					...
					disabled={
						cartActionLoading || selectedIngredients.length === 0
					}
