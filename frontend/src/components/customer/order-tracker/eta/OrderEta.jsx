...
const ETA = {
	...
	pickup: {
		...
		row2: {
			...
			value: (eta, durationMins) =>
				`${durationMins ?? eta?.travel?.durationMins ?? "--"}`,
			unit: "min",
	...
	if (!eta) {
		return (
			<div className="order__ready-wrapper">
				<p className="order__ready-label">
					{t("order_tracker.estimating")}
				</p>
			</div>
		);
	}
	...
	return (
		<div className="order__ready-wrapper">
			...
			<div className="order__ready">
				...
				<p className="order__ready-time">{etaUI?.row1?.value?.(eta)}</p>
