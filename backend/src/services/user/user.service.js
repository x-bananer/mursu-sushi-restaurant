...
export async function assertUserOwnsOrder(userId, orderId, locale) {
	...
	const ownsOrder = await userRepo.userOwnsOrder(normalizedUserId, normalizedOrderId);
...
export async function updateOwnProfile(userId, data, locale) {
	if (
		...
		throw createHttpError(403, t(locale, 'user', 'cannot_update_restricted_profile_fields'));
	}
