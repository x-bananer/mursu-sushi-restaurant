export function buildCombo(ingredientsFromClient, ingredientsFromDb, withValidation = false) {
	const ingredients = [...ingredientsFromClient].sort(
...
function validateIngredientsExist(ingredientsFromClient, ingredientsFromDb) {
	for (const clientItem of ingredientsFromClient) {
		const dbIngredient = ingredientsFromDb.find(
			(dbItem) => dbItem.id === clientItem.ingredient_id
		);
...
function validateItemsQuantity(sortedIngredientsFromClient, ingredientsFromDb) {
	const comboOrderError =
		'Invalid combo order: base must be at the bottom (exactly 1 required), fillings in the middle (max 3), toppings on top (max 1).';
	let baseCount = 0;
	let fillingCount = 0;
	let toppingCount = 0;
	...
	for (const clientIngredient of sortedIngredientsFromClient) {
		const dbIngredient = ingredientsFromDb.find(
			(dbIngredient) => Number(dbIngredient.id) === Number(clientIngredient.ingredient_id)
		);
