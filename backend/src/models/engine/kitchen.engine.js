...
function getItemPrepTime(item) {
	...
	if (item.item_type_id === 2) {
		...
		const time = PREP_CONFIG.custom.base + ingredientCount * PREP_CONFIG.custom.perIngredient;
...
function estimateQueueDelay(context = {}) {
	const { activeOrdersAheadCount = 0, avgOrderPrepTime = 10 } = context;

	if (activeOrdersAheadCount === 0) return 0;

	const throughputFactor = 0.6;

	return Math.round(activeOrdersAheadCount * avgOrderPrepTime * throughputFactor);
}
